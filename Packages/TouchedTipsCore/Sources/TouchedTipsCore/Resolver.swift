...
public enum Resolver {
    ...
    private static func gap(_ visit: Visit, _ add: ContactAdd) -> TimeInterval {
        if visit.end < add.seenStart { return add.seenStart.timeIntervalSince(visit.end) }
        if visit.start > add.seenEnd { return visit.start.timeIntervalSince(add.seenEnd) }
        return 0
    }
}
