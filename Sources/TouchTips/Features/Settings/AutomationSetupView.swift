...
struct AutomationSetupView: View {
    ...
    var body: some View {
        Form {
            Section {
                Text("When you leave Contacts or Phone, Shortcuts can ask TouchTips to check for new contacts without opening the app.")
                Text("This is not a contact-save trigger. NameDrop and contacts saved elsewhere are discovered at a later check. No background location is used.")
            ...
            Section {
                ...
                Text("The first successful check learns your existing contacts silently. Do this before saving a test contact. Later checks preserve your notes and meeting details. Recording works even without notifications.")
            }

            Section {
                Text("In Shortcuts, open Automation and create a new App automation.")
                Text("Choose Contacts (and Phone if you use it to add people), select Is Closed, and choose Run Immediately or turn off Ask Before Running.")
                ...
                Text("This button opens TouchTips’ actions in Shortcuts; it cannot install an automation. You must create the App trigger yourself. Setup is specific to this device.")
            }

            Section {
                Text("Save a new test contact in Contacts, then switch to another app. TouchTips should submit a notification if access is allowed. Focus and notification settings may silence it.")
                if lastShortcutCheck > 0 {
                    LabeledContent("Last shortcut check") {
                        Text(Date(timeIntervalSince1970: lastShortcutCheck), format: .dateTime.month().day().hour().minute().second())
                    }
                } else {
                ...
                Text("The timestamp proves the action completed, not that an automation is installed or that a banner appeared. Opening TouchTips also checks contacts, so look for the notification before returning here.")
            }
        }
